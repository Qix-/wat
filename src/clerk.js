...
const clerk = {
	...
	start(options) {
		...
		this.scaffold();
		this.load();
		indexer.init({clerk, updateRemotely: options.updateRemotely});
		setInterval(this.history.worker, 5000);
		setInterval(this.updater.nextQueueItem, 6000);
	},

	scaffold() {
		mkdirp.sync(path.join(__dirname, '/../.local'));
		mkdirp.sync(path.join(__dirname, '/../.local/docs'));
		this.scaffoldDocs();
		fs.appendFileSync(path.join(__dirname, '/../' + this.paths.prefs, ''));
		fs.appendFileSync(path.join(__dirname, '/../' + this.paths.cache, ''));
		fs.appendFileSync(path.join(__dirname, '/../' + this.paths.hist, ''));
		return this;
	},

	scaffoldDocs() {
		const index = this.index.index() || {};
		const dir = path.join(__dirname, '/../.local/docs/');
